...
stock SayText2(client, String:message[], size, bool:teamOnly=false, bool:silence=false)
{
	if (!silence)
	{
		...
		Format(format, sizeof(format), "\x01%s%s\x03%%s1 \x01:  %%s2", status_prefix, team_prefix);
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "");
		PrintToConsole(client, "Created by Twelve-60 of GameTech and fixed by Danyas");
		PrintToConsole(client, "");
